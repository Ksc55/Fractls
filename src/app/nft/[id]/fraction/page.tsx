    ...
    return (
      <>
        <div className="flex flex-col  h-screen">
          <section
            className={
              "w-100  bg-[#d9d9d9]  h-3/6 flex justify-center items center"
            }
          >
            <Image
              src={url}
          ...
          <section className={"flex justify-between w-full mt-6"}>
            <div>
              <div className={"grid grid-cols-1 gap-1"}>
                <span className="text-black/[.70] font-['Roboto'] text-[.9375rem] leading-[normal]">
                  Owned By
                </span>
                <span className="text-black font-['Roboto'] text-2xl leading-[normal]">
                  @Collector
                </span>
              </div>
            </div>
            <div>
              <div className=" w-[475px] h-16 rounded-[0.875rem] border border-black bg-white mt-4 flex justify-between items-center px-2">
                <div className={"flex flex-col"}>
                  <div className="text-black font-['Roboto'] text-2xl leading-[normal]">
                    {price} ETH
                  </div>
                </div>
                <button
                  className="flex-shrink-0 w-[7.1875rem] h-10 rounded-full bg-customGreen-50"
                  onClick={onBuy}
                >
                  Buy
                </button>
          ...
          <div className={"flex justify-center items-center my-20"}>
            <div className="text-[#121212] text-center font-['Roboto'] text-[2rem] font-semibold leading-[150%]">
              Similar F-NFTs
            </div>
          </div>
          <div className={"flex justify-between items-center"}>
            {randomNFTS.map((_nft) => (
              <RandomNFTCard nft={_nft.result} />
            ))}
          </div>
        </div>
      </>
...
const RandomNFTCard = ({ nft }) => {
  ...
  if (metadata) {
    ...
    return (
      <div
        className={
          "flex flex-col justify-center items-center border-2 border-stale-950"
        }
