import {PoolCard} from "@/components/PoolCard";
import StakeNFTCard from "@/components/StakeNFTCard";

export default function Page() {
    return (
        <div className="liquidity-pool">
            <div className="div">
                <div className="group-3">
                    <div className="frame-wrapper">
                        <div className="frame-2">
                            <div className="div-wrapper">
                                <div className="frame-3">
                                    <div className="frame-4">
                                        <PoolCard/>
                                        <a href="your_link_destination" className="frame-8">
                                            <div className="text-wrapper-11">View More Pools</div>
                                        </a>
                                    </div>
                                    <div className="group-10">
                                        <div className="group-11">
                                            <div className="frame-9">
                                                <div className="text-wrapper-12">Filter by APR</div>
                                                <img className="img-2" src="/arrow-down-6.svg"/>
                                            </div>
                                            <div className="frame-10">
                                                <div className="text-wrapper-12">Filter by Volume</div>
                                                <img className="img-2" src="/arrow-down-6.svg"/>
                                            </div>
                                            <div className="frame-11">
                                                <div className="text-wrapper-12">Filter by Lock Period</div>
                                                <img className="img-2" src="/arrow-down-4.svg"/>
                                            </div>
                                            <div className="frame-12">
                                                <div className="text-wrapper-12">Filter by Date</div>
                                                <img className="img-2" src="/arrow-down-4.svg"/>
                                            </div>
                                        </div>
                                        <div className="frame-13">
                                            <div className="text-wrapper-12">Search Pool</div>
                                            <img className="img-2" src="/arrow-right.svg"/>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="frame-14">
                                <div className="frame-15">
                                    <p className="p">
                                        <span className="span">Contribute Your Token To </span>
                                        <span className="text-wrapper-13">Pools</span>
                                        <span className="span"> &amp; Earn Rewards</span>
                                    </p>
                                    <div className="frame-16">
                                        <div className="group-12">
                                            <div className="group-13">
                                                <a href="link_to_your_destination" className="text-wrapper-14">Learn How
                                                    It Works</a>
                                                <img className="video-circle" src="/video-circle.svg"/>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div className="frame-17">
                                    <div className="group-14">
                                        <div className="group-15">
                                            <div className="text-wrapper-15">Total Value Locked (TVL)</div>
                                            <div className="text-wrapper-16">$870,000</div>
                                        </div>
                                        <div className="group-16">
                                            <div className="text-wrapper-15">Total Staked NFT’s</div>
                                            <div className="text-wrapper-17">17,000</div>
                                        </div>
                                        <div className="group-17">
                                            <div className="text-wrapper-15">Total Staked NFT’s</div>
                                            <div className="text-wrapper-17">17,000</div>
                                        </div>
                                        <div className="group-18">
                                            <div className="text-wrapper-15">Total Rewards Claimed</div>
                                            <div className="text-wrapper-17">17,000</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div className="group-19">
                    <div className="frame-18">
                        <div className="frame-15">
                            <p className="p">
                                <span className="text-wrapper-13">Stake</span> <span
                                className="span"> &amp; Earn High Yield</span>
                            </p>
                            <div className="frame-16">
                                <div className="group-12">
                                    <div className="group-13">
                                        <a href="link_to_your_destination" className="text-wrapper-14">Learn How It
                                            Works</a>
                                        <img className="video-circle" src="/video-circle.svg"/>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="frame-19">
                            <div className="group-20">
                                <div className="group-21">
                                    <div className="group-15">
                                        <div className="text-wrapper-15">Total Value Locked (TVL)</div>
                                        <div className="text-wrapper-16">$870,000</div>
                                    </div>
                                    <div className="group-22">
                                        <div className="text-wrapper-15">No of Stakers</div>
                                        <div className="text-wrapper-18">115,113</div>
                                    </div>
                                    <div className="group-23">
                                        <div className="text-wrapper-19">APR</div>
                                        <div className="text-wrapper-20">19.76%</div>
                                    </div>
                                    <div className="group-24">
                                        <div className="text-wrapper-15">Total Rewards Claimed</div>
                                        <div className="text-wrapper-17">17,000</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="frame-20">
                        <div className="frame-21">
                            <div className="group-25">
                                <div className="group-26">
                                    <div className="frame-22">
                                        <div className="text-wrapper-21">100</div>
                                        <div className="frame-23">
                                            <div className="text-wrapper-22">Matic</div>
                                            <img className="img-2" src="/arrow-down-3.svg"/>
                                        </div>
                                    </div>
                                    <div className="text-wrapper-23">I want to stake</div>
                                </div>
                                <div className="text-wrapper-24">Available Matic: 500</div>
                            </div>
                            <div className="group-27">
                                <div className="group-28">
                                    <div className="group-29">
                                        <div className="overlap-group-3">
                                            <div className="rectangle-2"></div>
                                            <div className="ellipse"></div>
                                        </div>
                                    </div>
                                    <div className="group-30">
                                        <div className="text-wrapper-25">7 Days</div>
                                        <div className="text-wrapper-26">Duration:</div>
                                        <div className="text-wrapper-27">14 Days</div>
                                        <div className="text-wrapper-28">21 Days</div>
                                        <div className="text-wrapper-29">28 Days</div>
                                    </div>
                                </div>
                            </div>
                            <div className="group-31">
                                <div className="text-wrapper-23">APR</div>
                                <div className="text-wrapper-30">100%</div>
                            </div>
                            <div className="group-32">
                                <div className="frame-24">
                                    <div className="text-wrapper-31">Stake</div>
                                </div>
                            </div>
                            <div className="frame-25">
                                <div className="frame-26">
                                    <div className="text-wrapper-32">Stake</div>
                                </div>
                                <div className="text-wrapper-33">Unstake</div>
                            </div>
                        </div>
                        <div className="group-33">
                            <div className="frame-27">
                                <div className="group-34">
                                    <div className="text-wrapper-15">POL/ETH</div>
                                    <div className="group-35">
                                        <div className="text-wrapper-23">APR</div>
                                        <div className="text-wrapper-30">100%</div>
                                    </div>
                                    <div className="group-36">
                                        <div className="text-wrapper-23">Amount Staked</div>
                                        <div className="text-wrapper-34">200.5</div>
                                    </div>
                                    <div className="group-37">
                                        <div className="text-wrapper-23">Duration</div>
                                        <div className="text-wrapper-35">21 Days</div>
                                    </div>
                                    <div className="group-38">
                                        <div className="group-39">
                                            <div className="text-wrapper-23">Transaction Fee</div>
                                            <div className="text-wrapper-36">0.1 Matic</div>
                                        </div>
                                    </div>
                                    <div className="group-40">
                                        <div className="group-39">
                                            <div className="text-wrapper-23">Rewards</div>
                                            <div className="text-wrapper-37">100 Matic</div>
                                        </div>
                                    </div>
                                </div>
                                <div className="frame-28">
                                    <div className="frame-26">
                                        <div className="text-wrapper-38">Summary</div>
                                    </div>
                                    <div className="text-wrapper-39">Analytics</div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div className="frame-29">
                    <div className="frame-30">
                        <div className="group-41">
                            <StakeNFTCard/>
                        </div>
                        <a href="your_link_destination" className="frame-33">
                            <div className="text-wrapper-11">View More NFT’s</div>
                        </a>
                    </div>
                    <div className="frame-34">
                        <div className="frame-15">
                            <p className="earn-attractive">
                                <span className="text-wrapper-43">Earn attractive yields, </span>
                                <span className="text-wrapper-13">Stake</span>
                                <span className="text-wrapper-43"> &amp; </span>
                                <span className="text-wrapper-44">Borrow</span>
                                <span className="text-wrapper-43">&nbsp;</span>
                                <span className="text-wrapper-45">NFT’s</span>
                                <span className="text-wrapper-43"> to leverage your portfolio. </span>
                            </p>
                            <div className="frame-35">
                                <div className="group-12">
                                    <div className="group-13">
                                        <a href="link_to_your_destination" className="text-wrapper-14">Learn How It
                                            Works</a>
                                        <img className="video-circle" src="/video-circle.svg"/>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="frame-36">
                            <div className="group-42">
                                <div className="group-15">
                                    <div className="text-wrapper-15">Total Value Locked (TVL)</div>
                                    <div className="text-wrapper-16">$870,000</div>
                                </div>
                                <div className="group-16">
                                    <div className="text-wrapper-15">Total Staked NFT’s</div>
                                    <div className="text-wrapper-17">17,000</div>
                                </div>
                                <div className="group-43">
                                    <div className="text-wrapper-15">Total Participating Wallets</div>
                                    <div className="text-wrapper-46">17,000</div>
                                </div>
                                <div className="group-44">
                                    <div className="text-wrapper-15">Total Rewards Claimed</div>
                                    <div className="text-wrapper-17">17,000</div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div className="frame-37">
                    <div className="frame-38">
                        <div className="group-45">
                            <div className="group-26">
                                <div className="frame-22">
                                    <div className="text-wrapper-21">100</div>
                                    <div className="frame-39">
                                        <div className="text-wrapper-22">Matic</div>
                                        <img className="img-2" src="/arrow-down-1.svg"/>
                                    </div>
                                </div>
                                <div className="text-wrapper-23">I want to stake</div>
                            </div>
                            <div className="group-46">
                                <div className="frame-22">
                                    <div className="frame-40">
                                        <div className="text-wrapper-22">ETH</div>
                                        <img className="arrow-down" src="/arrow-down-1.svg"/>
                                    </div>
                                    <div className="text-wrapper-21">100</div>
                                </div>
                                <div className="text-wrapper-23">I will earn</div>
                            </div>
                            <div className="group-47">
                                <div className="frame-22">
                                    <div className="text-wrapper-21">100%</div>
                                </div>
                                <div className="text-wrapper-23">APR</div>
                            </div>
                            <div className="group-48">
                                <div className="group-28">
                                    <div className="group-29">
                                        <div className="overlap-group-3">
                                            <div className="rectangle-2"></div>
                                            <div className="ellipse"></div>
                                        </div>
                                    </div>
                                    <div className="group-30">
                                        <div className="text-wrapper-25">7 Days</div>
                                        <div className="text-wrapper-26">Duration:</div>
                                        <div className="text-wrapper-27">14 Days</div>
                                        <div className="text-wrapper-28">21 Days</div>
                                        <div className="text-wrapper-29">28 Days</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="text-wrapper-47">Yield Calculator</div>
                        <a href="your_link_destination" className="frame-41">
                            <div className="text-wrapper-48">Estimate Yield</div>
                        </a>
                    </div>
                    <p className="estimate-your-token">
                        <span className="text-wrapper-49">Estimate </span>
                        <span className="span">Your Token&nbsp;&nbsp;Rewards When You Contribute To Pools</span>
                    </p>
                    <p className="text-wrapper-50">
                        This helps you decide the trades you would participate in and plan your asset allocation.
                    </p>
                    <img className="image-2" src="/image-31.png"/>
                </div>
                <div className="frame-42">
                    <div className="text-wrapper-51">Home</div>
                    <img className="arrow-left-2" src="/arrow-left.svg"/>
                    <div className="text-wrapper-51">Liquidity Pool</div>
                </div>
            </div>
        </div>
    )
}
