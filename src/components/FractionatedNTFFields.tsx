...
const FractionatedNTFFields: React.FC<FractionatedNTFFieldsProps> = ({
  image,
  setValue,
}) => {
  ...
  const handleInputChange = (index: number, value: string) => {
    ...
    setValues(newValues);
    setValue(
      newValues.map((value, index) => ({ image: shards[index], value }))
    );
  };
    ...
    <div>
      <div className="grid grid-cols-3 gap-4">
        {shards.map((shard, index) => (
          <div
            key={index}
            className="flex flex-col justify-center w-22 justify-self-center gap-4"
          >
            <NextImage
              src={shard}
              alt={`Shard ${index + 1}`}
