...
export default function WalletButton() {
  ...
  const styleButton =
    "font-bold border-2 border-customGreen-50 rounded-full px-4 h-10 text-base";
  if (isConnected && mounted) {
    return (
      <div className={"flex items-center  gap-2"}>
        ...
        <button className={styleButton} onClick={() => disconnect()}>
          DISCONNECT
        </button>
      </div>
    );
  ...
  return (
    <button className={styleButton} onClick={() => connect()}>
      CONNECT WALLET
    </button>
  );
}
